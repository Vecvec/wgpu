#[cfg(custom)]
use crate::custom;
use crate::{dispatch, BindGroup, Buffer, Label, QuerySet, RayTracingPipeline};
...
impl RayTracingPass<'_> {
    ...
    /// Dispatches ray tracing work operations.
    ///
    /// `x`, `y` and `z` denote the number of rays to trace in each dimension.
    pub fn trace_rays(&mut self, x: u32, y: u32, z: u32) {
        self.inner.trace_rays(x, y, z);
    }

    /// Dispatches ray tracing work operations indirectly.
    ///
    /// `x`, `y` and `z` denote the number of rays to trace in each dimension.
    pub fn trace_rays_indirect(
        ...
        self.inner
            .trace_rays_indirect(&indirect_buffer.inner, indirect_offset);
    }
