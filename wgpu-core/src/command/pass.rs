...
/// An error encountered while performing a function available on all passes.
...
pub(crate) fn set_bind_group(
    ...
    if index >= max_bind_groups {
        return Err(PassError::BindGroupIndexOutOfRange {
            index,
            max: max_bind_groups,
        });
    }
...
pub(crate) fn rebind_resources<F: FnOnce()>(
    state: &mut BaseState,
            ...
            for (i, e) in entries.iter().enumerate() {
                if let Some(group) = e.group.as_ref() {
                    let raw_bg = group
                        ...
                        .map_err(PassError::DestroyedResource)?;
                    unsafe {
                        state.raw_encoder.set_bind_group(
                            pipeline_layout.raw(),
...
pub(crate) fn set_push_constant<F: FnOnce(&[u32])>(
    state: &mut BaseState,
    ...
    let values_offset = values_offset.ok_or(PassError::InvalidValuesOffset)?;
    ...
    let pipeline_layout = state
        ...
        .pipeline_layout
        .as_ref()
        .ok_or(PassError::MissingPipeline)?;
